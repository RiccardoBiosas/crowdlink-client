...
contract CrowdlinkReferral is Ownable {
    ...
    event NewCampaignOpen(uint date, string website, address owner, uint256 budget, uint256 reward);
    ...
    function addInfluencer(string memory _website, string memory _referral_link, address _owner) public returns(bool) {
        (uint referral_campaign_indx, bool found)= helper_selectReferralCampaign(_owner, _website);
        //also check if it's still isActive
        
        if(found) {
            uint256 reward = referral_campaign_collection[_owner][referral_campaign_indx].campaignReward;
            ...
            referral_campaign_collection[_owner][referral_campaign_indx].referral_campaign_referral_links_list.push(_referral_link);
    ...
    function openReferralCampaign(uint _campaignBudget, uint _campaignReward, string memory _website ) payable public returns(uint256){
        require(msg.value == _campaignBudget && (account_balance[msg.sender] + msg.value) > account_balance[msg.sender]);
        ...
        account_balance[msg.sender] = account_balance[msg.sender].add(msg.value);
        referral_campaign_collection[msg.sender].push(referral_campaign_instance);
        
        emit NewCampaignOpen(now, _website, msg.sender, _campaignBudget, _campaignReward);
    ...
    function getBalanceOfInfluencer(string memory _link) view public returns (uint256) {
        ...
        string memory influencer_instance_link = influencer_instance.referral_link;
    ...
    function forwardRewards(string memory _referral_link, string memory _website, address _from, address _to ) public returns(uint256 _campaign_owner_balance, uint256 _influencer_balance) { //add isOwner modifier
        (uint referral_campaign_indx, bool found)= helper_selectReferralCampaign(_from, _website);

        require(found == true && account_balance[_from] > influencers[_referral_link].reward && referral_campaign_collection[_from][referral_campaign_indx].isActive == true && (account_balance[_from] - influencers[_referral_link].reward) < account_balance[_from]);
        
        if(referral_campaign_collection[_from][referral_campaign_indx].remainingBudget > influencers[_referral_link].reward ) {
            referral_campaign_collection[_from][referral_campaign_indx].remainingBudget = referral_campaign_collection[_from][referral_campaign_indx].remainingBudget.sub(influencers[_referral_link].reward);
            account_balance[_from] = account_balance[_from].sub(influencers[_referral_link].reward);
            account_balance[_to] = account_balance[_to].add(influencers[_referral_link].reward);
            
            return(account_balance[_from], account_balance[_to]);
                
        } else {
            referral_campaign_collection[_from][referral_campaign_indx].isActive = false; //should you remove a campaign once it's not active anymore?
            return(account_balance[_from], account_balance[_to]);
        }
    }
    
    function withdrawFromCampaign(string memory _website) public payable returns(uint256) {
        (uint referral_campaign_indx, bool found) = helper_selectReferralCampaign(msg.sender, _website);
        
        require(found == true && referral_campaign_collection[msg.sender][referral_campaign_indx].remainingBudget > 0 && account_balance[msg.sender] > 0 && account_balance[msg.sender] >= referral_campaign_collection[msg.sender][referral_campaign_indx].remainingBudget);
        referral_campaign_collection[msg.sender][referral_campaign_indx].isActive = false;
        account_balance[msg.sender] = account_balance[msg.sender].sub(referral_campaign_collection[msg.sender][referral_campaign_indx].remainingBudget);
        msg.sender.transfer(referral_campaign_collection[msg.sender][referral_campaign_indx].remainingBudget);
        referral_campaign_collection[msg.sender][referral_campaign_indx].remainingBudget = 0;
    ...
    function lookUpCampaignReferral(address _account_address, uint256 _index) public view returns(string memory, uint256, uint256, bool, address[] memory)  {
        ...
        address[] memory referral_campaign_influencers_address_list = new address[](referral_campaign_collection[_account_address][_index].referral_campaign_referral_links_list.length); //length of the array equal to length of the referral campaign's referral links array
        for(uint i=0; i<referral_campaign_collection[_account_address][_index].referral_campaign_referral_links_list.length; i++) {
            
            address influencer_addr = influencers[referral_campaign_collection[_account_address][_index].referral_campaign_referral_links_list[i]].addr;
